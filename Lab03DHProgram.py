...
class SecurePRNG:
    ...
    def generate(self, n_bytes):
        ...
        random_bytes = random.randbytes(n_bytes)
        output = b""
...
def xor_crypt(data, prng):
    ...
    output = b""
    byte_length = prng.generate(len(data))
    for i in range(len(data)):
        output += bytes([data[i] ^ byte_length[i]])
    return output
...
def main():
    ...
    message = b"Hello" # Put in your test message here
